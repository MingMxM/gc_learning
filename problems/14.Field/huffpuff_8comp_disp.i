...
#
# Cycle (single):
#   Inject  : 0   - 30  day   (inlet held at p_inject = 45 MPa, ACID solution)
...
#
# NOTE: rock compressibility included (PorousFlowPorosity, fluid=true) to aid soak convergence.
...
[Materials]
  ...
  [permeability_fracture]
    ...
    block = fracture
  []
  # Dispersion tensor: one diffusion coefficient per fluid component (8).
  # disp_long / disp_trans are set on the PorousFlowDispersiveFlux kernels.
  [diffusivity]
...
# coexisting with the advection kernels the Action generates. Increase
# disp_trans to spread solutes further into the matrix (transverse to the
# along-fracture flow).
# --------------------------------------------------------------------------
[Kernels]
  [disp_H]
...
# Each timestep: this main app solves transport and saves per-node rate_*
# (kg/s). The react sub-app (serpentinization_geochemistry_field.i) receives
# them as pf_rate_*, solves the reaction at every node, and returns updated
# mass fractions massfrac_* which overwrite f0..f6 here.
# temperature (temp) is also passed for the reaction kinetics.
# ==========================================================================
[MultiApps]
  [react]
...
[Transfers]
  ...
  [changes_due_to_flow]
    ...
    to_multi_app = react
  []
  # sub -> main : updated mass fractions from the reaction
